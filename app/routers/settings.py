...
def _current(db: Session, user_id: int) -> schemas.SettingsRead:
    return schemas.SettingsRead(
        ...
        preferred_genres_anime=crud.get_preferred_genres(db, user_id, "anime"),
...
def update_settings(
    ...
    if payload.preferred_genres_anime is not None:
        crud.set_preferred_genres(db, user.id, "anime", payload.preferred_genres_anime)
